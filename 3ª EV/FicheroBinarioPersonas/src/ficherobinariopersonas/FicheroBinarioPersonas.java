...
public class FicheroBinarioPersonas{
	...
	static int menu(Scanner e,int tm) {
		...
		case 0:
			...
			System.out.println("3.B�squeda personas.");
		...
		try {
			...
			while(!nombre.equals("**")){
				...
				if(nombre.equals("**")) { 
					...
					do {
						System.out.println("Sexo (V = Var�n/M = mujer)? ");
					...
					//if(nombre.equals("**")) { si pones aqu� el if sigue con el alta pero no ser�a grabada 
	...
	static void busquedas(int tb) throws IOException {
		...
		if(tb==1) {		
			do {
				try {
					System.out.println("Teclee sexo�?Sexo (V = Var�n/M = mujer)? ");
		...
		//EOFException eofe se puede usar pero como data lee datos y no objetos, por lo que aqu� usamos el null que vale igualmente.
	}
	static void fin() {
		System.out.println("\n\tFIN DEL PROGRAMA\n\t================");
	}

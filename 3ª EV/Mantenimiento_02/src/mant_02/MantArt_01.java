...
public class MantArt_01{
	...
	void bajas() throws IOException{
		...
		a.leerDeArchivo(fich);
		if(a.getCodigo()==0)
			System.out.println("\n\tEl art�culo con el c�digo "+codBus+" no existe.....\n");
		else {
			a.mostrarDatos(2);
			do {
	...
	void modificaciones() throws IOException{
		...
		do {
			System.out.println("\n\tMODIFICACIONES\n\t");
			do {
				System.out.print("\nTeclee n�mero del art�culo: ");
				codBus= t.leerInt();
			}while(codBus==Integer.MIN_VALUE);
			fich.seek(codBus * a.tamano());
			a.leerDeArchivo(fich);
			if(a.getCodigo()==0)
				System.out.println("\n\tEl art�culo con el n�mero "+codBus+" no existe.....\n");
			else {
				...
				do {
					...
					case 1:
						do {
							System.out.println("Teclee la nueva denominaci�n....");
							denomN=t.leerString();
					...
					case 3:
						do {
							do {
								System.out.print("Teclee el nuevo stock m�nimo.....");
					...
					case 4:
						do {
							do {
								System.out.print("Teclee el nuevo stock m�ximo.....");
			...
			do {
				System.out.println("Modificar otro art�culo (s/n)");
				otro=Character.toLowerCase(t.leerChar());
			}while(otro!='s' && otro!='n');
		}while(otro=='s');
		fich.close();
	...
	void listadoGeneral() throws IOException{
		...
		int cp=0,cl=LINEAS+1 ; //ca cont aprobados cp cont p�ginas cs cont suspensos y cl cont l�neas vale 6 para saltar
		RandomAccessFile fich = new RandomAccessFile(ruta+"articulos.dat","r");
		System.out.println("Posici�n de puntero inicial..........:"+fich.getFilePointer());
		do {
		...
		do {
			...
			if(!fin) {
				...
				System.out.println("C�digo\tDenominaci�n\tStock actual\tStock m�nimo.\tStock m�ximo\t\tApto");
				System.out.println("-----------------------------------------------------------------------");
				cl=0;
			}			
		}while(!fin);
	...
	void listadoEntreLimites() throws IOException {
		...
		final int LINEAS= 4;
		//valor 5 de nombre LINEAS por convenio es mayuscula.
		int cp=0,cl=LINEAS+1,li=0,ls=0; //ca cont aprobados cp cont p�ginas cs cont suspensos y cl cont l�neas vale 6 para saltar
		do {
			do {
		...
		do {
			while(!fin && cl < LINEAS) {
				...
				do {
					fin=a.leerDeArchivo(fich);
				}while(a.getCodigo()==0 &&!fin); //sigue leyendo mientras el c�digo no sea 0 o fin, se sale del bucle en el siguiente registro que exista
				if(a.getCodigo()>ls) {
					fin=true;
				}
			}
			...
			if(!fin) {
				System.out.println("\t\tLISTADO ENTRE L�MITES\t\tPag.: "+ ++cp+"\n\t\t======================\n");
				System.out.println("C�digo\tDenominaci�n\tStock actual\tStock m�nimo.\tStock m�ximo\t\tApto");
				System.out.println("-----------------------------------------------------------------------");
				cl=0;
			}	
	...
	void listadoPedidos() throws IOException{
		...
		do {
			...
			while(!fin && cl < LINEAS) {
				...
				if(cl==LINEAS) {
					sumsigue+=tp;
					System.out.println("Total p�gina..............: "+tp);
				...
				if(!fin) {
					...
					System.out.println("C�digo\tDenominaci�n\tUnidades Pedidas\tPrecio Unidad\tValor pedido");
					System.out.println("-------------------------------------------------------------------");
					cl=0;
				}	
			}

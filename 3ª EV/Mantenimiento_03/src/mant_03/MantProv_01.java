...
public class MantProv_01{
	...
	public static void main(String []args) throws IOException {
		...
		fich.close(); // asï¿½ te aseguras con certeza de que el fichero existe.-
		do {
			op=mp.menu();
			switch(op) {
		...
		while(numero!=0) { // si no quieres salir no tecleas cero.
			...
			if(p.getNumero()!=0)
				...
				if(confirmar=='s') {
					...
					while(numero*p.tamano()>fich.length()) //mientras la direcciï¿½n sea mayor que la longitud
						pv.grabarEnArchivo(fich); //como no puede haber espacios se crean registros en blanco (av es el archivo vacï¿½o)
					fich.seek(numero * p.tamano()); // ya se dejï¿½ de cumplir el while por lo que ya estoy posicionado.

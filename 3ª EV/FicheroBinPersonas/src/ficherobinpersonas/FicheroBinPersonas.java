...
public class FicheroBinPersonas{
	...
	static String convertirLetraMayuscula(String dni) {
		String n=dni.substring(0,8); //coges el n�mero
		n+=Character.toUpperCase(dni.charAt(8)); //lo concatenas con la letra en may�scula.
		return n;
	...
	static void altas(Scanner e) throws IOException{
		...
		try {
			...
			while(!dni.equalsIgnoreCase("999")){
				do {
					System.out.println("Nombre (m�ximo 20 caracteres)? ");
					nombre = e.nextLine();
	...
	static void consultas(Scanner e) {
		...
		do {
			...
			try {
				do {
					System.out.println("\tRealizar otra b�squeda (s/n)? ");
	...
	static char menuListados(Scanner e) {
		...
		System.out.println("F. Volver al men� principal.");
		do {
			System.out.println("\t\t Teclee opci�n(G/V/M/E/F)�?");
			op=e.next().charAt(0);
		}while("GVMEF".indexOf(op)==-1); /*si esta letra est� te indica la posici�n y si no est� te devuelve -1.*/
		switch(op) {
		case 'g':
			listadoGeneral();
			break;
	...
	static int mediaEdadSexo(char sexo) {
		...
		if(sexo=='V') {
			...
			System.out.println("N�mero de hombres: "+c+"  \nMedia de su edad: "+(media/c));
		}
		if(sexo=='M') {
			try {
				DataInputStream ficheroEnt= new DataInputStream(new FileInputStream(ruta + "gente.dat"));
			...
			System.out.println("N�mero de personas: "+c+"  \nMedia de su edad: "+(media/c));
		}	
		return c;
	}
	static void mayorMenor(char t) {
		...
		case 'v':
			...
			System.out.println("El hombre m�s joven tiene:" +menor);
			System.out.println("El hombre m�s mayor tiene:" +mayor);
			break;
		case 'g':
			try {
				DataInputStream ficheroEnt= new DataInputStream(new FileInputStream(ruta + "gente.dat"));
			...
			System.out.println("La mujer m�s joven tiene:" +menor);
			System.out.println("La mujer m�s mayor tiene:" +mayor);
			break;
		case 'm':
			try {
				DataInputStream ficheroEnt= new DataInputStream(new FileInputStream(ruta + "gente.dat"));
			...
			System.out.println("La persona m�s joven tiene:" +menor);
			System.out.println("La persona m�s mayor tiene:" +mayor);
			break;
		}
	}
...
System.out.println("");
	...
	static void listadoEdad()  {
		...
		char sexo=' ';
		do {
			System.out.println("Teclee una edad m�nima");
			edadMin=e.nextInt();
			System.out.println("Teclee una edad m�xima");
			edadMax=e.nextInt();

...
public class Repaso{
	...
	static void altas(Scanner e)  {
		...
		try {
			...
			// el true es para a�adir nuevas personas.
			p= new Persona();
			do {
			...
			while(!p.getDni().equalsIgnoreCase("fin")) {
				...
				do {
					System.out.println("Teclee un c�digo postal");
				...
				esc.newLine();
				e.nextLine(); //saltar l�nea para vaciar buffer.
	...
	static void listado() {
		...
		System.out.println("DNI\tNombre\tEdad\tC�d.Postal");
		try {
		...
		do {
			...
			if(ip==vp.length)
				...
				do {
					...
					default:
						do {
							System.out.println("Teclee el nuevo c�digo postal");
							try {

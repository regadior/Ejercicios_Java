...
public class Ficheros_Binarios2 {
    static final String ruta = "D:\\Ejercicios_Java\\Ficheros_Binarios\\src";
        ...
        try {
            ...
            while (!fin) {
                try {
                    ...
                    entrada.close(); // da igual cerrarlos aquí o donde está comentado porque la excepción lo que
                                     // controla es el fin del fichero.
                    salida.close();
                    fin = true;
                }
            }
    ...
    static void listarFicheroInicial() throws IOException {
        ...
        try {
            ...
            // DataInputStream entrada= new DataInputStream(new FileInputStream(ruta +
            // "\\personas"+i+".dat"));
            // Data y Object no se pueden usar a la vez, el object guarda en objetos(como
            // son las variables) y el Data en variables primitivas(como son los String)
            nombre = entrada.readUTF();
            while (!fin) {
                edad = entrada.readInt();
                System.out.println("Nombre: " + nombre + "\tedad: " + edad);
